...
def no_heads_up_display() -> None:
    print(
        "Cannot run the Ready Trader Go heads-up display. This could\n"
        ...
        for path in args.autotrader:
            if path.suffix.lower() == ".py":
                ...
                pool.apply_async(
                    subprocess.run,
                    ([resolved],),
                    {"check": True, "cwd": resolved.parent},
                    error_callback=lambda e: on_error("Auto-trader '%s'" % path, e),
                )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Ready Trader Go command line utility."
    )
    ...
    replay_parser = subparsers.add_parser(
        ...
        aliases=["re"],
        description=(
            "View a replay of a Ready Trader Go match from " " a match events file."
        ),
    ...
    replay_parser.add_argument(
        "filename",
        nargs="?",
        default=pathlib.Path("match_events.csv"),
