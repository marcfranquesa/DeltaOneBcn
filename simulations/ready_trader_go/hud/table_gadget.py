...
class BasicPriceLadderGadget(BaseTableGadget):
    ...
    def set_model(self, table_model: BasicPriceLadderModel):
        ...
        if self._table_view.model() is not None:
            self._table_view.model().best_ask_row_changed.disconnect(
                self.update_best_ask_row
            )

        self._table_view.setModel(table_model)
        self.setWindowTitle(
            "Future" if table_model.instrument == Instrument.FUTURE else "ETF"
        )
...
class ProfitLossTableGadget(BaseTableGadget):
    ...
    def __init__(
        ...
        self.select_team_label.setText(
            "Select a team to see their active orders and trade history:"
        )
    ...
    def __selection_changed(
        ...
        self.__profit_model.on_selection_changed(
            self.__proxy_model.mapSelectionToSource(selected),
    ...
    def set_model(self, table_model: ProfitLossTableModel) -> None:
        ...
        self._table_view.sortByColumn(
            self.__profit_model.NET_PROFIT_COLUMN, Qt.SortOrder.DescendingOrder
        )
