...
class EventSource(QtCore.QObject):
    ...
    midpoint_price_changed = QtCore.Signal(
        Instrument, float, float
    )  # instrument, time, price in cents

    order_amended = QtCore.Signal(
        str, float, int, int
    )  # team, time, order_id, volume delta
    ...
    order_inserted = QtCore.Signal(
        str, float, int, Instrument, Side, int, int, Lifespan
    )
...
class LiveEventSource(EventSource):
    ...
    def on_message(self, typ: int, data: bytes, length: int):
        ...
        if typ == MessageType.AMEND_EVENT and length == AMEND_EVENT_MESSAGE_SIZE:
            ...
            self.event_source_error_occurred.emit(
                "received invalid message: length=%d type=%d" % (length, typ)
            )

    def on_error_message(self, client_order_id: int, error_message: bytes):
        """Callback when an error message is received."""

    def on_amend_event_message(
        self, now: float, competitor_id: int, order_id: int, volume_delta: int
    ) -> None:
        """Callback when an amend event message is received."""
        self.__now = now
        order = self.__orders[competitor_id].get(order_id)
        if order is not None:
            self.__order_books[order.instrument].amend(
                now, order, order.volume + volume_delta
            )
            if order.remaining_volume == 0:
                del self.__orders[competitor_id][order_id]
        if competitor_id != 0:
            self.order_amended.emit(
                self.__teams[competitor_id], now, order_id, volume_delta
            )

    def on_cancel_event_message(
        self, now: float, competitor_id: int, order_id: int
    ) -> None:
    ...
    def on_insert_event_message(
        ...
        if competitor_id != 0:
            self.order_inserted.emit(
                self.__teams[competitor_id],
                now,
                order_id,
                Instrument(instrument),
                Side(side),
    ...
    def on_hedge_event_message(
        ...
        self.__now = now
        self.__accounts[competitor_id].transact(
            Instrument(instrument), Side(side), price, volume, 0
        )
    ...
    def _on_timer_tick(self):
        ...
        for i in Instrument:
            ...
            if midpoint_price is not None:
                ...
                self.__order_books[i].top_levels(
                    self.__ask_prices,
                    self.__ask_volumes,
                    self.__bid_prices,
                    self.__bid_volumes,
        ...
        if future_price is not None and etf_price is not None:
            for competitor_id, account in self.__accounts.items():
                ...
                self.profit_loss_changed.emit(
                    self.__teams[competitor_id],
                    self.__now,
                    account.profit_or_loss / 100.0,
                    account.etf_position,
                    account.future_position,
                    account.account_balance / 100.0,
    ...
    def on_trade_event_message(
        ...
        self.__now = now
        self.__accounts[competitor_id].transact(
            Instrument(instrument), Side(side), price, volume, fee
        )
        self.trade_occurred.emit(
            self.__teams[competitor_id], now, order_id, Side(side), volume, price, fee
        )
...
class RecordedEventSource(EventSource):
    ...
    def _on_timer_tick(self):
        ...
        for i in Instrument:
            if len(self.__order_books[i]) >= (tick + 1) * 4 * TOP_LEVEL_COUNT:
                data = (
                    self.__order_books[i][
                        j * TOP_LEVEL_COUNT : (j + 1) * TOP_LEVEL_COUNT
                    ]
    ...
    def from_csv(
        ...
        accounts: Dict[str, CompetitorAccount] = collections.defaultdict(
            source._account_factory.create
        )
        ...
        def take_snapshot(when: float):
            for i in Instrument:
                events.append(
                    Event(
                        when,
                        source.midpoint_price_changed.emit,
                        (i, when, books[i].midpoint_price()),
                    )
                )
                books[i].top_levels(ask_prices, ask_volumes, bid_prices, bid_volumes)
                source.__order_books[i].extend(
                    itertools.chain(ask_prices, ask_volumes, bid_prices, bid_volumes)
                )
            ...
            if future_price is not None and etf_price is not None:
                for team, account in accounts.items():
                    ...
                    events.append(
                        Event(
                            when,
                            source.profit_loss_changed.emit,
                            (
                                team,
                                when,
                                account.profit_or_loss / 100.0,
        ...
        for row in reader:
            ...
            if operation == "Insert":
                ...
                events.append(
                    Event(
                        tm,
                        source.order_amended.emit,
                        (team, tm, order_id, volume_delta),
                    )
                )
                ...
                events.append(
                    Event(tm, source.order_cancelled.emit, (team, tm, order_id))
                )
                ...
                if operation == "Trade":
                    if (
                        order_id in orders[team]
                        and orders[team][order_id].remaining_volume == 0
                    ):
                        del orders[team][order_id]
                    events.append(
                        Event(
                            tm,
                            source.trade_occurred.emit,
                            (team, tm, order_id, side, volume, price, fee),
                        )
                    )
