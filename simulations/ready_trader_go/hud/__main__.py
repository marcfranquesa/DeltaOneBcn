...
def __show_splash() -> QtWidgets.QSplashScreen:
    splash = QtWidgets.QSplashScreen(
        QtGui.QPixmap(str(HERE.joinpath("images/splash.png")))
    )
    splash.show()
    return splash
...
def __show_main_window(
    splash: QtWidgets.QSplashScreen, event_source: EventSource
) -> MainWindow:
    splash.showMessage(
        "Creating main window...", Qt.AlignBottom, QtGui.QColor("#F0F0F0")
    )
...
def __validate_configuration(filename: pathlib.Path, config: Mapping[str, Any]) -> bool:
    if type(config) is not dict:
        print(
            "%s: expected JSON object got %s" % (filename, type(config).__name__),
            file=sys.stderr,
        )
        return False
    ...
    for key in ("EtfClamp", "TickSize"):
        if key not in instrument:
            print(
                "%s: missing '%s' element in Instrument section" % (filename, key),
                file=sys.stderr,
            )
            return False
        ...
        if type(instrument[key]) is not float:
            print(
                "%s: expected float got %s" % (key, type(instrument[key]).__name__),
                file=sys.stderr,
            )
            return False
...
def replay(path: pathlib.Path):
    ...
    splash.showMessage(
        "Processing %s..." % str(path), Qt.AlignBottom, QtGui.QColor("#F0F0F0")
    )
