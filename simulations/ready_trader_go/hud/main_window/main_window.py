...
from ready_trader_go.hud.table_gadget import (
    BasicPriceLadderGadget,
    PerTeamTableGadget,
    ProfitLossTableGadget,
)
from ready_trader_go.hud.table_model import (
    ActiveOrderTableModel,
    BasicPriceLadderModel,
    ProfitLossTableModel,
...
class MainWindow(Ui_main_window, QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        event_source.event_source_error_occurred.connect(
            self.__on_event_source_error_occurred
        )
    ...
    def __show_sub_window(
        self,
        gadget: QtWidgets.QWidget,
        callback: Callable,
    ...
    def __show_all_teams_profit_table(self) -> None:
        ...
        self.__show_sub_window(
            self.__pnl, self.__on_all_teams_profit_table_closed, 0.49, 0.4
        )
    ...
    def __show_etf_dynamic_depth(self) -> None:
        ...
        self.__show_sub_window(
            self.__etf_ladder, self.__on_etf_dynamic_depth_closed, 0.30, 0.4
        )
    ...
    def __show_future_dynamic_depth(self) -> None:
        ...
        self.__show_sub_window(
            self.__future_ladder, self.__on_future_dynamic_depth_closed, 0.21, 0.4
        )
    ...
    def __show_team_active_order_table(self) -> None:
        ...
        self.__show_sub_window(
            self.__aov, self.__on_team_active_order_table_closed, 0.45, 0.3
        )
    ...
    def __show_team_trade_history_table(self) -> None:
        ...
        self.__show_sub_window(
            self.__tv, self.__on_team_trade_history_table_closed, 0.45, 0.3
        )
    ...
    def __show_midpoint_price_chart(self) -> None:
        ...
        self.event_source.midpoint_price_changed.connect(
            self.__mcg.on_midpoint_price_changed
        )
        self.__show_sub_window(
            self.__mcg, self.__on_midpoint_price_chart_closed, 0.55, 0.3
        )
    ...
    def __show_profit_loss_chart(self) -> None:
        ...
        self.event_source.profit_loss_changed.connect(
            self.__pnl_chart.on_profit_loss_changed
        )
        for team in self.__team_volumes.keys():
            self.__pnl_chart.on_login_occurred(team)
        self.__show_sub_window(
            self.__pnl_chart, self.__on_profit_loss_chart_closed, 0.55, 0.3
        )
    ...
    def __setup_menus(self) -> None:
        ...
        self.etf_dynamic_depth_action.setStatusTip(
            "Reopen the ETF dynamic depth window"
        )
        self.etf_dynamic_depth_action.triggered.connect(self.__show_etf_dynamic_depth)

        self.future_dynamic_depth_action.setStatusTip(
            "Reopen the Future dynamic depth window"
        )
        self.future_dynamic_depth_action.triggered.connect(
            self.__show_future_dynamic_depth
        )

        self.all_teams_profit_table_action.setStatusTip("Reopen the profit loss table")
        self.all_teams_profit_table_action.triggered.connect(
            self.__show_all_teams_profit_table
        )

        self.team_active_orders_table_action.setStatusTip(
            "Reopen the active orders table"
        )
        self.team_active_orders_table_action.triggered.connect(
            self.__show_team_active_order_table
        )

        self.team_trade_history_table_action.setStatusTip(
            "Reopen the trade history table"
        )
        self.team_trade_history_table_action.triggered.connect(
            self.__show_team_trade_history_table
        )

        self.midpoint_price_chart_action.setStatusTip("Reopen the midpoint price chart")
        self.midpoint_price_chart_action.triggered.connect(
    ...
    def __setup_models(self) -> None:
        ...
        self.event_source.order_book_changed.connect(
            self.__future_model.update_order_book
        )
        ...
        self.event_source.profit_loss_changed.connect(
            self.__pnl_model.on_profit_loss_changed
        )
