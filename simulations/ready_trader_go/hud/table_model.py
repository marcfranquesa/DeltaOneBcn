...
class ActiveOrderTableModel(BaseTableModel):
    ...
    def __update_order_volume(self, order_id: int, volume_delta: int) -> None:
        row = next(
            (
                ...
                if self.__orders[i][self._ORDER_ID_COLUMN] == order_id
            ),
            None,
        )
        if row is not None:
    ...
    def on_order_amended(
        self, team: str, _: float, order_id: int, volume_delta: int
    ) -> None:
...
class BasicPriceLadderModel(BaseTableModel):
    ...
    def __append_rows(self, count: int) -> None:
        self.beginInsertRows(
            QtCore.QModelIndex(), self._row_count, self._row_count + count - 1
        )
    ...
    def update_order_book(
        ...
        min_best_ask: int = (
            best_ask if best_ask < self.last_best_ask else self.last_best_ask
        )
        max_best_ask: int = (
            best_ask if best_ask > self.last_best_ask else self.last_best_ask
        )
        ...
        if self._row_count - best_ask_row < self._MINIMUM_EXTRA_ROW_COUNT:
            self.__append_rows(
                best_ask_row + self._MINIMUM_EXTRA_ROW_COUNT * 2 - self._row_count
            )
        ...
        self.__ask_volumes.update(
            (p, "{:,}".format(v)) for p, v in zip(ask_prices, ask_volumes) if p
        )
        self.dataChanged.emit(
            self.createIndex(0, self._ASK_VOL_COLUMN),
            self.createIndex(min_best_ask_row, self._ASK_VOL_COLUMN),
        )

        self.__bid_volumes.clear()
        self.__bid_volumes.update(
            (p, "{:,}".format(v)) for p, v in zip(bid_prices, bid_volumes) if p
        )
        self.dataChanged.emit(
            self.createIndex(max_best_bid_row, self._BID_VOL_COLUMN),
...
class PriceLadderModel(BasicPriceLadderModel):
    ...
    _COLUMN_TOOLTIPS = (
        "Bid volume for the selected team",
        "Total bid volume available at each price level",
        None,
        "Total ask volume available at each price level",
    ...
    def set_competitor_model(self, team_volumes):
        ...
        self.dataChanged.emit(
            self.createIndex(0, self.TEAM_ASK_COLUMN),
...
class TeamLadderVolumes:
    ...
    def on_order_amended(
        self, team: str, now: float, order_id: int, volume_delta: int
    ) -> None:
        """Callback when an order is amended."""
        if team == self.team:
    ...
    def on_order_cancelled(self, team: str, now: float, order_id: int) -> None:
        ...
        if team == self.team:
            if order_id in self.__ask_orders:
                self.__subtract_volume(
                    order_id, self.__ask_orders[order_id].remaining_volume
                )
            elif order_id in self.__bid_orders:
                self.__subtract_volume(
                    order_id, self.__bid_orders[order_id].remaining_volume
                )
...
class ProfitLossTableModel(BaseTableModel):
    ...
    def on_profit_loss_changed(
        ...
        self.__profits[row][1:] = (
            etf_position,
            fut_position,
            profit - account_balance,
            account_balance + total_fees,
            -total_fees,
            profit,
        )
        self.dataChanged.emit(
            self.createIndex(row, 1), self.createIndex(row, len(self._COLUMN_NAMES) - 1)
        )
