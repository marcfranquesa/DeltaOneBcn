...
class BaseChartGadget(QtWidgets.QWidget):
    ...
    def _scroll_x_axis(self, time: float) -> None:
        ...
        if time > self.__x_axis_maximum:
            ...
            self.chart.scroll(
                scroll_distance * self.chart.plotArea().width() / CHART_DURATION, 0
            )
...
class MidpointChartGadget(BaseChartGadget):
    ...
    def on_midpoint_price_changed(
        self, instrument: Instrument, time: float, mid_price: float
    ) -> None:
...
class ProfitLossChartGadget(BaseChartGadget):
    ...
    def __init__(self, parent: Optional[QtWidgets.QWidget] = None):
        ...
        self.team_series: Dict[str, QtCharts.QSplineSeries] = collections.defaultdict(
    ...
    def on_login_occurred(self, team: str) -> None:
        ...
        line_series.setColor(
            self._COLOURS[(len(self.team_series) - 1) % len(self._COLOURS)]
        )
