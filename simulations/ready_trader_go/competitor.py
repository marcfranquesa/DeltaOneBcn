...
class Competitor(ICompetitor, IOrderListener):
    ...
    def __init__(
        self,
        name: str,
        exec_channel: IExecutionConnection,
        etf_book: OrderBook,
    ...
    def on_order_amended(self, now: float, order: Order, volume_removed: int) -> None:
        ...
        if self.exec_connection is not None:
            self.exec_connection.send_order_status(
                ...
                order.volume - order.remaining_volume,
    ...
    def on_order_filled(
        self, now: float, order: Order, price: int, volume: int, fee: int
    ) -> None:
        ...
        if not (
            -self.position_limit <= self.account.etf_position <= self.position_limit
        ):
    ...
    def on_unhedged_lots_expiry(self):
        ...
        self.logger.info(
            "Unhedged lots timer expired for %s at etf=%d fut=%d rel=%d",
            self.name,
            self.account.etf_position,
            self.account.future_position,
    ...
    def on_amend_message(self, now: float, client_order_id: int, volume: int) -> None:
        ...
        if client_order_id > self.last_client_order_id:
            self.send_error(
                now, client_order_id, b"out-of-order client_order_id in amend message"
            )
            return
        ...
        if client_order_id in self.orders:
            ...
            if volume > order.volume:
                self.send_error(
                    now, client_order_id, b"amend operation would increase order volume"
                )
            else:
    ...
    def on_cancel_message(self, now: float, client_order_id: int) -> None:
        ...
        if client_order_id > self.last_client_order_id:
            self.send_error(
                now, client_order_id, b"out-of-order client_order_id in cancel message"
            )
            return
    ...
    def on_hedge_message(
        self, now: float, client_order_id: int, side: int, price: int, volume: int
    ) -> None:
        """Called when a hedge order request is received from the competitor."""
        if client_order_id <= self.last_client_order_id:
            self.send_error(
                now, client_order_id, b"duplicate or out-of-order client_order_id"
            )
            return
        ...
        if price % self.tick_size != 0:
            self.send_error(
                now, client_order_id, b"price is not a multiple of tick size"
            )
            return
        ...
        if now == 0.0:
            self.send_error(
                now, client_order_id, b"order rejected: market not yet open"
            )
            return
        ...
        if volume_traded == 0:
            ...
            best: Optional[int] = (
                self.future_book.best_ask()
                if side_ == Side.BID
                else self.future_book.best_bid()
            )
            if best is None:
                ...
                if last_traded is None:
                    self.send_error(
                        now,
                        client_order_id,
                        b"order rejected: cannot determine future price",
                    )
                    return
                if (side_ == Side.ASK and last_traded >= price) or (
                    side_ == Side.BID and last_traded <= price
                ):
        ...
        self.account.update(
            self.future_book.last_traded_price() or self.future_book.midpoint_price(),
        ...
        if not (
            -self.position_limit <= self.account.future_position <= self.position_limit
        ):
            self.hard_breach(now, client_order_id, b"future position limit breached")

    def on_insert_message(
        self,
        now: float,
        client_order_id: int,
        side: int,
        ...
        if lifespan != Lifespan.FILL_AND_KILL and lifespan != Lifespan.GOOD_FOR_DAY:
            self.send_error(
                now, client_order_id, b"%d is not a valid lifespan" % lifespan
            )
            return
        ...
        if self.active_volume + volume > self.active_volume_limit:
            self.send_error(
                now,
                client_order_id,
                b"order rejected: active order volume limit breached",
            )
            return
        ...
        if (
            side == Side.BUY and self.sell_prices and price >= -self.sell_prices[-1]
        ) or (side == Side.SELL and self.buy_prices and price <= self.buy_prices[-1]):
            self.send_error(
                now, client_order_id, b"order rejected: in cross with an existing order"
            )
            return

        order = self.orders[client_order_id] = Order(
            client_order_id,
            Instrument.ETF,
            Lifespan(lifespan),
            Side(side),
    ...
    def on_timer_tick(self, now: float, future_price: int, etf_price: int) -> None:
        ...
        self.score_board.tick(
            now, self.name, self.account, etf_price, future_price, self.status
        )
    ...
    def send_error(self, now: float, client_order_id: int, message: bytes) -> None:
        ...
        self.logger.info(
            "'%s' sent error message: time=%.6f client_order_id=%s message='%s'",
            self.name,
            now,
            client_order_id,
            message.decode(),
    ...
    def send_error_and_close(
        self, now: float, client_order_id: int, message: bytes
    ) -> None:
        """Send an error message to the auto-trader and shut down the match."""
...
class CompetitorManager:
    ...
    def __init__(
        ...
        traders_config: Dict[str, str],
        account_factory: AccountFactory,
        etf_book: OrderBook,
        future_book: OrderBook,
        match_events: MatchEvents,
        score_board_writer: ScoreBoardWriter,
        tick_size: float,
        timer: Timer,
        unhedged_lots_factory: UnhedgedLotsFactory,
    ):
    ...
    def login_competitor(
        self, name: str, secret: str, exec_channel: IExecutionConnection
    ) -> Optional[ICompetitor]:
        """Return the competitor object for the given name."""
        if (
            name in self.__competitors
            or name not in self.__traders
            or self.__traders[name] != secret
        ):
            return None
        ...
        competitor = Competitor(
            ...
            self.__etf_book,
            self.__future_book,
            self.__account_factory.create(),
            self.__match_events,
            self.__score_board_writer,
            self.__position_limit,
            self.__order_count_limit,
            self.__active_volume_limit,
            self.__tick_size,
            self.__unhedged_lots_factory,
            self.controller,
        )
