...
class Order:
    ...
    def __repr__(self) -> str:
        result = (
            "Order(order_id=%d, side=%s, price_in_cents=%d, volume=%d, lifespan=%s)"
        )
        return result % (
            self.order_id,
            self.side,
...
class DemoClient:
    ...
    def display_error(message: str) -> None:
        ...
        print(
            ...
            "Please quit and restart Ready Trader Go and restart this Jupyter notebook."
            % message
        )
    ...
    def connect(self, host: str = "127.0.0.1", port: int = 12345) -> None:
        ...
        self.__sock.send(
            HEADER.pack(LOGIN_MESSAGE_SIZE, MessageType.LOGIN)
    ...
    def send_amend_order(self, order_id: int, new_volume: int) -> None:
        ...
        """
        self.__sock.send(
            HEADER.pack(AMEND_MESSAGE_SIZE, MessageType.AMEND_ORDER)
    ...
    def send_cancel_order(self, order_id: int) -> None:
        ...
        self.__sock.send(
            HEADER.pack(CANCEL_MESSAGE_SIZE, MessageType.CANCEL_ORDER)
    ...
    def send_insert_order(
        ...
        self.__sock.send(
            HEADER.pack(INSERT_MESSAGE_SIZE, MessageType.INSERT_ORDER)
    ...
    def __on_order_status_message(
        self, order_id: int, fill_volume: int, remaining_volume: int, fees: int
    ) -> None:
        order: Order = self.__orders[order_id]
