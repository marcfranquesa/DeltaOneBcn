...
class InformationPublisher(asyncio.DatagramProtocol):
    ...
    def __init__(
        ...
        self.__send_ticks_handles: List[Optional[asyncio.Handle]] = [
            None for _ in Instrument
        ]
        ...
        HEADER.pack_into(
            self.__ticks_message, 0, TRADE_TICKS_MESSAGE_SIZE, MessageType.TRADE_TICKS
        )
    ...
    def on_timer_tick(self, timer: Timer, now: float, tick_number: int) -> None:
        ...
        for book in self.__order_books:
            ...
            ORDER_BOOK_HEADER.pack_into(
                self.__book_message, HEADER_SIZE, book.instrument, tick_number
            )
            ORDER_BOOK_MESSAGE.pack_into(
                self.__book_message,
                ORDER_BOOK_HEADER_SIZE,
                *self.__ask_prices,
                *self.__ask_volumes,
    ...
    def on_trade(self, book: OrderBook) -> None:
        ...
        if self.__send_ticks_handles[book.instrument] is None:
            self.__send_ticks_handles[book.instrument] = self.__event_loop.call_soon(
                self.__send_trade_ticks, book
            )
    ...
    def __send_trade_ticks(self, order_book: OrderBook) -> None:
        ...
        if order_book.trade_ticks(
            self.__ask_prices, self.__ask_volumes, self.__bid_prices, self.__bid_volumes
        ):
            self.__trade_ticks_sequences[order_book.instrument] += 1
            TRADE_TICKS_HEADER.pack_into(
                self.__ticks_message,
                HEADER_SIZE,
                order_book.instrument,
                self.__trade_ticks_sequences[order_book.instrument],
            )
            TRADE_TICKS_MESSAGE.pack_into(
                self.__ticks_message,
                TRADE_TICKS_HEADER_SIZE,
                *self.__ask_prices,
                *self.__ask_volumes,
