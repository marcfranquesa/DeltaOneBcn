...
async def __start_autotrader(
    auto_trader: BaseAutoTrader, config: Dict[str, Any], loop: asyncio.AbstractEventLoop
) -> None:
...
def main(name: str = "autotrader") -> None:
    ...
    app.event_loop.create_task(
        __start_autotrader(auto_trader, app.config, app.event_loop)
    )
    app.run()
