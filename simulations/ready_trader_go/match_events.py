...
class MatchEventsWriter:
    ...
    def __init__(
        self, match_events: MatchEvents, filename: str, loop: asyncio.AbstractEventLoop
    ):
        """Initialise a new instance of the MatchEvents class."""
    ...
    def on_writer_done(self, num_events: int) -> None:
        ...
        for c in self.task_complete:
            c(self)
        self.logger.info(
            "writer thread complete after processing %d match events", num_events
        )
    ...
    def start(self):
        ...
        try:
            ...
            self.logger.error(
                "failed to open match events file: filename=%s",
                self.filename,
                exc_info=e,
            )
            raise
        else:
            self.writer_task = threading.Thread(
                target=self.writer,
                args=(match_events_file,),
                daemon=False,
                name="match_events",
            )
    ...
    def writer(self, match_events_file: TextIO) -> None:
        ...
        try:
            with match_events_file:
                ...
                csv_writer.writerow(
                    (
                        "Time,Competitor,Operation,OrderId,Instrument,Side,Volume,Price,Lifespan,"
                        "Fee"
                    ).split(",")
                )
