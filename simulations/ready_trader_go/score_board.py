...
class ScoreRecord:
    ...
    def __init__(
        ...
        etf_position: int,
        future_position,
        etf_price: Optional[int],
        future_price: Optional[int],
        total_fees: int,
        balance: int,
        profit_loss: int,
...
class ScoreBoardWriter:
    ...
    def on_writer_done(self, num_events: int) -> None:
        ...
        for c in self.task_complete:
            c(self)
        self.logger.info(
            "writer thread complete after processing %d score records", num_events
        )
    ...
    def start(self):
        ...
        try:
            ...
            self.logger.error(
                "failed to open score board file: filename=%s",
                self.filename,
                exc_info=e,
            )
            raise
    ...
    def writer(self, score_records_file: TextIO) -> None:
        ...
        try:
            with score_records_file:
                ...
                csv_writer.writerow(
                    (
                        "Time,Team,Operation,BuyVolume,SellVolume,EtfPosition,FuturePosition,"
