                            ...
                            (
                                (
                                    (
                                        ...
                                        + ((bid_prices[2] * bid_volumes[2]) * self.l2_w)
                                        + ((bid_prices[3] * bid_volumes[3]) * self.l3_w)
                                        + ((bid_prices[4] * bid_volumes[4]) * self.l4_w)
                                        + ((ask_prices[0] * ask_volumes[0]) * self.l0_w)
                                        + ((ask_prices[1] * ask_volumes[1]) * self.l1_w)
                                        + ((ask_prices[2] * ask_volumes[2]) * self.l2_w)
                                        + ((ask_prices[3] * ask_volumes[3]) * self.l3_w)
                                        + ((ask_prices[4] * ask_volumes[4]) * self.l4_w)
                                    )
                                    / (
                                        (bid_volumes[0] * self.l0_w)
                                        + (bid_volumes[1] * self.l1_w)
                                        + (bid_volumes[2] * self.l2_w)
                                        + (bid_volumes[3] * self.l3_w)
                                        + (bid_volumes[4] * self.l4_w)
                                        + (
                                            ask_volumes[0] * self.l0_w
                                            + (ask_volumes[1] * self.l1_w)
                                            + (ask_volumes[2] * self.l2_w)
                                            + (ask_volumes[3] * self.l3_w)
                                            + (ask_volumes[4] * self.l4_w)
                                        )
                                    )
                                )
                            )
